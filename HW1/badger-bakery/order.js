...
function calculateSubtotal() {
	...
	for (const item of ITEMS) { // subtotal of order
		const quantity = parseInt(document.getElementById(item + "-quantity").value); // number of items
		const price = parseFloat(document.getElementById(item + "-price").innerText); // cost of item
...
function calculateSalesTax() {
	...
	let state_tax = getSalesTaxRateForState(state);
...
document.getElementById("btn-checkout").addEventListener("click", () => { // event listener to btn-checkout
